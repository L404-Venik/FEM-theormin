...
\centerline{\Huge \textbf{Метод Конечных Элементов}}
...
\begin{definition}
	\textbf{Локальный вектор нагрузки} $e^{(i)}$
	\[
...
\[
	\zeta_j = \dfrac{a_j x + b_j y + c_j}{2S}; \quad
	a_j = y_{j+1} - y _{j+2}, \quad 
	b_j = x_{j+2} - x_{j+1}, \quad 
	c_j = x_{j+1} y_{j+2} - x_{j+2} y_{j+1} \quad j = 1,2,3.
\]
...
\begin{definition}
	\textbf{Метод сходится}, если в некоторой норме $||u^h - u|| \xrightarrow[h \to 0]{} 0$, где $h$ - диаметр разбиения.
\end{definition}

\begin{definition}
	\textbf{Метод сходится со скоростью} $\mathcal{O}(h^k)$ $(k\geqslant 0)$, если $||u^h - u|| = \mathcal{O}(h^k), h \to 0$.
\end{definition}
...
\begin{definition}
	Функция $i_h v(x)$: $i_h v(x_i)= v(x_i)$ называется \textbf{(линейным) интерполянтом} функции $v(x)$, где $i_h v(x) \in S_1^h$, $v(x) \in C(\overline{I})$.
\end{definition}
